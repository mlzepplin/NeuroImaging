...
y_train_full =[];
training_input =zeros((18*144*144),18);
...
img_train_textured = (img_train_textured/max_int);
 ...
 for j=1:numcolumns;
     for i=1:numrows;
         ...
         %MAKING VEC_TRAIN A ROW VECTOR FOR COMPATABILITY WITH SVM
         vec_train = vec_train';
...
%TRAINING SVM
svm_struct = svmtrain(training_input,y_train_full,'method','SMO');
disp('training done');
...
for m=1:17;
 ...
 for i=1:numrows;
   for j=1:numcolumns;
     ...
     %size(test_vec)
     prediction = svmclassify(svm_struct,test_vec_extended');
     img_test(i,j) = prediction;
   end
 end
 
 %img_test = imfill(img_test,'holes');
