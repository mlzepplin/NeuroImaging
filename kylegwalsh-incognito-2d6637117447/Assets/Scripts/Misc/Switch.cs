...
public class Switch : MonoBehaviour {
    ...
    void Start() {
        ...
        if (!hackable) {
            ...
            hackButton.FindChild("Background").GetComponent<Image>().color = Color.white;
        }
    ...
    void ToggleNumberStatus(Number number, bool recursion) {
        ...
        if (number.status) {
            StartCoroutine(FadeAlphaFromToValue(number.transform.FindChild("HackDotActive").GetComponent<Image>(), 0f, 1f, fadeDuration));
        }
        else {
            StartCoroutine(FadeAlphaFromToValue(number.transform.FindChild("HackDotActive").GetComponent<Image>(), 1f, 0f, fadeDuration));
        }
    ...
    public void ButtonCallbackHack() {
        ...
        if (hackMode) {
            ...
            foreach (Number number in numbers) {
                StartCoroutine(FadeAlphaFromToValue(number.transform.FindChild("Label").GetComponent<Text>(), 1f, 0f, 0.25f));
            }

            // Secondly, fade in the circles for the hacking mini-game.
            foreach (Number number in numbers) {
                StartCoroutine(FadeAlphaFromToValue(number.transform.FindChild("HackDotInactive").GetComponent<Image>(), 0f, 1f, 0.25f, 0.25f));
            }

            // Lastly, activate a random number of hacking dots.
            foreach (Number number in numbers) {
                if (Random.value > 0.5f) {
                    StartCoroutine(FadeAlphaFromToValue(number.transform.FindChild("HackDotActive").GetComponent<Image>(), 0f, 1f, 0.5f, 0.5f));
                    number.status = true;
                }
            }
            ...
            foreach (Number number in numbers) {
                if (number.status) {
                    StartCoroutine(FadeAlphaFromToValue(number.transform.FindChild("HackDotActive").GetComponent<Image>(), 1f, 0f, 0.25f));
                    number.status = false;
                }
            }
            ...
            foreach (Number number in numbers) {
                StartCoroutine(FadeAlphaFromToValue(number.transform.FindChild("HackDotInactive").GetComponent<Image>(), 1f, 0f, 0.25f, 0.25f));
            }

            // Lastly, fade in the numbers.
            foreach (Number number in numbers) {
                StartCoroutine(FadeAlphaFromToValue(number.transform.FindChild("Label").GetComponent<Text>(), 0f, 1f, 0.5f, 0.5f));
            }
