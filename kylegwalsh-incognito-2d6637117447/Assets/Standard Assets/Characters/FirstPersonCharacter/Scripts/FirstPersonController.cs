...
namespace UnityStandardAssets.Characters.FirstPerson
{
    ...
    public class FirstPersonController : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            playerBody = this.gameObject.transform.Find("PlayerBody").gameObject;
        }
        ...
        void ScaleCapsuleForCrouching()
        {
            ...
            if (m_CharacterController.isGrounded && !m_Crouching)
            {
                ...
                //adjusts playersBody
                Vector3 newPlayerBodyHeight;
                newPlayerBodyHeight = playerBody.transform.localPosition;
                newPlayerBodyHeight.y += m_CapsuleHeight / 5f;
                playerBody.transform.localPosition = newPlayerBodyHeight;
                //crouches
                m_Crouching = true;
            }
            //trying to uncrouch
            ...
            {
                Ray crouchRay = new Ray(Vector3.up * m_CharacterController.radius * k_Half, Vector3.up);
        ...
        private void GetInput(out float speed)
        {
            ...
            // player is attempting to run while not crouched
            if(!m_IsWalking && !m_Crouching){
                speed = m_RunSpeed;
            }
            // player is attempted to run while crouched
            else if(!m_IsWalking && m_Crouching){
                ScaleCapsuleForCrouching();
                PreventStandingInLowHeadroom();
                if(!m_Crouching){
                    speed = m_RunSpeed;
